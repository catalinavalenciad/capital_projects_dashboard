...
-- Views for Tableau dashboard and analytical reporting

-- View 1: Budget variance by project
...
-- View 2: Change order cost impact by project
CREATE VIEW vw_change_order_impact AS
SELECT
    p.project_id,
    p.project_name,
...
-- View 3: RFI volume and average response time by project
...
-- View 4: Change event breakdown by type and reason
CREATE VIEW vw_change_event_breakdown AS
SELECT
    event_type,
    reason,
...
-- View 5: Submittal status summary by project
CREATE VIEW vw_submittal_summary AS
SELECT
    p.project_id,
    p.project_name,
    COUNT(s.submittal_id) AS total_submittals,
    SUM(CASE WHEN s.status = 'Approved' THEN 1 ELSE 0 END) AS approved,
    SUM(CASE WHEN s.status = 'Approved as Noted' THEN 1 ELSE 0 END) AS approved_as_noted,
    SUM(CASE WHEN s.status = 'Revise and Resubmit' THEN 1 ELSE 0 END) AS revise_resubmit,
    SUM(CASE WHEN s.status = 'Rejected' THEN 1 ELSE 0 END) AS rejected,
    SUM(CASE WHEN s.status = 'Pending' THEN 1 ELSE 0 END) AS pending
FROM projects p
JOIN submittals s ON p.project_id = s.project_id
GROUP BY p.project_id, p.project_name
ORDER BY total_submittals DESC;

-- View 6: Planned vs actual spend over time by project
CREATE VIEW vw_spend_over_time AS
SELECT
    p.project_name,
    bt.period_date,
    bt.planned_spend,
